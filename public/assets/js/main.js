 ...
 -----------------------------------------------------------------*/
  function mainNav() {
    ...
    $(".menu-item-has-children")
      .not(".nav-submenu-managed")
      .append('<span class="ak-munu_dropdown_toggle"></span>');
...
--------------------------------------------------------------*/
 ...
 --------------------------------------------------------------*/
  ...
  if ($.exists(".anim-title-2")) {
    let animTitle2 = $(".anim-title-2")
      .not(".catering-gsap-root .anim-title-2")
      .toArray();
  ...
  if ($.exists(".page-title-anim")) {
    var pageTitleAnimEls = $(".page-title-anim")
      .not(".catering-gsap-root .page-title-anim")
      .toArray();
    if (pageTitleAnimEls.length) {
      gsap.from(pageTitleAnimEls, 1, {
        y: 100,
        ease: "power4.out",
        delay: 1.5,
        stagger: {
 ...
 --------------------------------------------------------------*/
  ...
  /**
   * Catering page (Next.js client route): titles + hero backgrounds run here so they
   * work after client-side navigation; excluded from global blocks above via .catering-gsap-root.
   */
  window.hariInitCateringGsap = function () {
    var $root = $(".catering-gsap-root");
    if (!$root.length) return;
    if ($root.data("hari-gsap")) return;
    $root.data("hari-gsap", 1);

    $root.find("[data-src]").each(function () {
      var src = $(this).attr("data-src");
      if (!src) return;
      var safe = String(src).replace(/\\/g, "/").replace(/"/g, '\\"');
      $(this).css({
    ...
    var pageTitles = $root.find(".page-title-anim").toArray();
    if (pageTitles.length) {
      gsap.from(pageTitles, 1, {
        y: 100,
        ease: "power4.out",
        delay: 1.5,
        stagger: {
    ...
    $root.find(".anim-title-2").each(function () {
      var element = this;
      if (element.querySelector(".char")) return;
      var animTitleChar = new SplitText(element, {
        type: "chars ,words",
      });
    ...
    if (typeof ScrollSmoother !== "undefined") {
      ...
      if (sm && typeof sm.refresh === "function") {
        sm.refresh();
      }
    }
  };
